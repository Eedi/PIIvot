    ...
    for seq in digit_sequences:
        if len(seq) >= 3:
            for i in range(len(seq) - 2):
                if (
                    seq[i + 1] == str(int(seq[i]) + 1)
                    and seq[i + 2] == str(int(seq[i + 1]) + 1)
                ) or (
                    seq[i + 1] == str(int(seq[i]) - 1)
                    and seq[i + 2] == str(int(seq[i + 1]) - 1)
                ):
...
class LabelAnonymizationManager:
    ...
    def name_feedback_func(self, original_text, anonymized_text):
        feedback = ""
        if (
            Levenshtein.distance(original_text, anonymized_text)
            <= self.distance_threshold
        ):
            feedback = self.append_feedback(
                feedback,
                f"{anonymized_text} is too similar to {original_text}. Ensure the anonymized replacement is a different name.",
            )

        similar_words_feedback = self.get_similar_words_feedback(
            "NAME", original_text, anonymized_text
        )
    ...
    def phone_number_feedback_func(self, original_text, anonymized_text):
        ...
        if (
            ...
            feedback = self.append_feedback(
                feedback,
                f"{anonymized_text} is too similar to {original_text}. Ensure the anonymized replacement is a different phone number.",
            )

        if any(char.isalpha() for char in anonymized_text):
            feedback = self.append_feedback(
                feedback,
                f"{anonymized_text} contains alphabetic characters. Ensure the anonymized phone number doesn't have any placeholder characters.",
            )

        if contains_counting_sequence(anonymized_text):
            feedback = self.append_feedback(
                feedback,
                f"{anonymized_text} contains ascending or descending strings of digits. Ensure the anonymized phone number doesn't look fake.",
            )

        similar_words_feedback = self.get_similar_words_feedback(
            "PHONE_NUMBER", original_text, anonymized_text
        )
        if similar_words_feedback:
            feedback = self.append_feedback(feedback, similar_words_feedback)
    ...
    def location_address_feedback_func(self, original_text, anonymized_text):
        ...
        if (
            ...
            feedback = self.append_feedback(
                feedback,
                f"{anonymized_text} is too similar to {original_text}. Ensure the anonymized replacement is a different word.",
            )

        similar_words_feedback = self.get_similar_words_feedback(
            "LOCATION_ADDRESS", original_text, anonymized_text
        )
        if similar_words_feedback:
            feedback = self.append_feedback(feedback, similar_words_feedback)
    ...
    def school_name_feedback_func(self, original_text, anonymized_text):
        ...
        for school_stage_identifier in school_stage_identifiers:
            if (
                school_stage_identifier in original_text.lower()
                and school_stage_identifier not in anonymized_text.lower()
            ):
                feedback = self.append_feedback(
                    feedback,
                    f"{original_text} refers to a {school_stage_identifier} stage school. Ensure the anonymized school name also uses the word {school_stage_identifier}.",
                )

        similar_words_feedback = self.get_similar_words_feedback(
            "SCHOOL_NAME", original_text, anonymized_text
        )
        if similar_words_feedback:
            feedback = self.append_feedback(feedback, similar_words_feedback)
    ...
    def date_of_birth_feedback_func(self, original_text, anonymized_text):
        ...
        if (
            ...
            feedback = self.append_feedback(
                feedback,
                f"{anonymized_text} is too similar to {original_text}. Ensure the anonymized replacement is a different day, month, and/or year.",
            )

        similar_words_feedback = self.get_similar_words_feedback(
            "DATE_OF_BIRTH", original_text, anonymized_text
        )
        if similar_words_feedback:
            feedback = self.append_feedback(feedback, similar_words_feedback)
    ...
    def get_similar_words_feedback(self, label_name, original_text, anonymized_text):
        ...
        for prior_text in prior_mappings:
            if prior_text != original_text:
                similar_words = self.get_similar_words(prior_text, original_text)
                prior_mapped_text = prior_mappings[prior_text]

                anonymized_similar_words = self.get_similar_words(
                    prior_mapped_text, anonymized_text
                )
    ...
    def get_anoymization_feedback(self, label_name, original_text, anonymized_text):
        ...
        feedback = self.feedback_func_dictionary.get(
            label_name, self.default_feedback_func
    ...
    def set_prior_label_mappings(self, prior_mappings):
        self.prior_mappings = prior_mappings
        self.set_mappings = prior_mappings
