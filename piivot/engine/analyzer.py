...
class Analyzer():
    ...
    def process_batch(self, 
                      ...
                      context_groups: List[str]) -> pd.DataFrame:
        """
    ...
    def analyze(self, 
        ...
        for i, data_column in enumerate(reversed(data_columns)):
            ...
            if data_column not in df.columns:
                warnings.warn(f"Column '{data_column}' does not exist in the input dataframe. Skipping...")
        ...
        for original_data_column, data_column in zip(original_data_columns, data_columns):
            ...
            if context_groups:
                df.drop(columns=[data_column, f"window_start_{original_data_column}", f"window_end_{original_data_column}"], inplace=True)
